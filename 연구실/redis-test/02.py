...
def determine_dict(params_aof, params_rdb, params_activedefrag, params_maxmemory,params_etc, params_dict):
    ...
    if persis_choice == 'aof':  ## chocie AOF
        ...
        params_dict = {**params_aof}

    elif persis_choice == 'rdb':  ## choice RDB

        params_dict = {**params_rdb}

    activedefrag_choice = random_choice(['yes', 'no'])
    if activedefrag_choice == 'yes':
        params_activedefrag['activedefrag'][2] = 'yes'
        params_dict = {**params_activedefrag}
...
params_maxmemory = {
    "maxmemory":['numerical_categorical', ['1gb', '2gb', '3gb', '4gb', '5gb', '6gb', '7gb', '8gb', '9gb', '10gb'], None],
    "maxmemory policy":["categorical",["volatile-lru","allkeys-lru","volatile-lfu","alkeys-lfu","volatile-random","allkeys-random","volatile-ttl","noeviction"],None],
